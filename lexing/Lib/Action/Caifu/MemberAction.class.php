<?php
class MemberAction extends CaifubaseAction {
	...
	public function atlas(){
		...
		$id = (int)I('id');
		$member_list = array();
		$member_model = new CfMemberModel();
		if ($id>0) {
		...
		$member_list['area_nums'] = $member_model->newAreaNums($member_list['id']); //推荐体系人数
	...
	protected function member($member_model,$mid,&$member_list,$level=0) {
		...
		foreach ($member_l as $row){
			...
			$row['area_nums'] = $member_model->newAreaNums($row['id']); //推荐体系人数
