 ...
 */
// 用户模型
class CfMemberModel extends Model {
	...
	public function addByMgr($data=array(),$leveldata=array()) {
		...
		$data['level'] = 0;
		/**node_str**************************************************/
		$p_node_str = $this->where('id='.$data['parent_aid'])->getField('node_str');
		$data['node_str'] = $p_node_str.$data['parent_aid'].',';
		/************************************************************/
		...
		$levelup_M = new CfLevelupModel(); $config_M = new CfConfigModel();
	...
	public function newAreaNums($id) {
		...
		$condition['node_str'] = array('like','%,'.$id.',%');
		return $this->where($condition)->count();
	}
